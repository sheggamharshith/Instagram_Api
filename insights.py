...
if __name__ == "__main__":
	...
	try:
		...
		logger.info("---- LATEST POST -----") # section header
		logger.info(f"Link to post: {response['json_data']['data'][0]['permalink']}") # link to post
		logger.info(f"Post caption: {response['json_data']['data'][0]['caption']}") # post caption
		logger.info(f"Media Type: {response['json_data']['data'][0]['media_type']}") # type of media
		logger.info(f"Posted at: {response['json_data']['data'][0]['timestamp']}") # when it was posted
		...
		# logger.info( "---- LATEST POST INSIGHTS -----") # section header

		# for insight in response['json_data']['data'] : # loop over post insights
		# 	logger.info( "\t" + insight['title'] + " (" + insight['period'] + "): " + str( insight['values'][0]['value'] )) # display info

		response = getUserInsights( params ) # get insights for a user

		logger.info("---- DAILY USER ACCOUNT INSIGHTS -----") # section header

		for insight in response['json_data']['data'] : # loop over user account insights
			logger.info("\t" + insight['title'] + " (" + insight['period'] + "): " + str( insight['values'][0]['value'] ) )# display info

			for value in insight['values']:
				logger.info("\t\t" + value['end_time'] + ": " + str( value['value'])) # logger.info( out counts for the date

	except Exception as e:
		logger.error(f"There is some error in insight module {e}")